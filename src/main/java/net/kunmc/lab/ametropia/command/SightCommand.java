...
public class SightCommand {
    ...
    private static int setClientValue(CommandSource src, SightChangeMessage.Type type, float value) {
        if (type != null)
            PacketHandler.INSTANCE.send(PacketDistributor.ALL.noArg(), new SightChangeMessage(type, value));
